...
if __name__ == '__main__':
    ...
    config=[100, 1, 0.1, 0.5]
    # 输入层数据维度784，隐藏层100，输出层10，lamda=1，learningratemin=0.1，learningratemax=0.5
    dl = MLP([784, config[0], 10], config[1], config[2], config[3])
    # 循环训练方法
    w1,w2,b1,b2,x,costlist0,costlist,acclist=dl.train(x_train, y_train)
    # 预测模型
    acc=dl.predict(x_test, y_test)
    # print('准确率：{0}%'.format(100*acc))
    save_the_model(w1,w2,b1,b2,config)
    draw_the_curve(x,costlist0,costlist,acclist)
    ...
    # w2降维至hdnum*3 1张size*size三通道图片可视化w2(图片size需要根据隐藏神经元数量自行确定，下面可视化hdnum为完全平方数时的情况)
    size=int(hdnum**0.5)
    data = w2.T
    pca = PCA(3)
    pca.fit(data)
